...
```mermaid
graph TB
    %% 上方：车机 DUT 和 PC 控制端（并排）
    subgraph TopDevices["外部设备"]
        direction LR
        PC_CMD["PC (控制端)<br/>(软件控制)"]:::pccmd
    ...
    classDef p1 fill:#FFE4B5,stroke:#DAA520,stroke-width:2px,color:#000
    ...
    classDef p4 fill:#DDA0DD,stroke:#9370DB,stroke-width:2px,color:#000
    classDef pc fill:#F08080,stroke:#DC143C,stroke-width:2px,color:#000
    classDef pccmd fill:#98D8C8,stroke:#20B2AA,stroke-width:2px,color:#000
...
- **P1 (PC端)**: MODE_PC 时与 P0 导通，用于 ADB 调试（P0 ↔ P1）

- **P2 (U盘1端)**: MODE_UDISK1 时与 P0 导通，车机读取 U 盘1（P0 ↔ P2）

- **P3 (U盘2端)**: MODE_UDISK2 时与 P0 导通，车机读取 U 盘2（P0 ↔ P3，可选）

- **P4 (CMD 控制接口)**: USB CDC Virtual COM 端口，用于软件控制、配置和状态查询，独立于数据切换链路，始终连接
...
| 数据传输速率 | 480 Mbps |
| 端口数量 | 1 个公共端 + 3 个切换端口（PC + U盘1 + U盘2）+ 1 个控制端口 |
| 供电方式 | USB 供电 (5V) |
...
| P1: TO_PC_UPSTREAM | USB Type-C | 连接 PC（ADB 调试） |
| P2: TO_UDISK1_DOWNSTREAM | USB Type-A | 连接 U 盘1（下行） |
| P3: TO_UDISK2_DOWNSTREAM | USB Type-A | 连接 U 盘2（下行，可选） |
| P4: CMD 控制接口 | USB Type-C  (USB CDC Virtual COM) | 用于软件控制 |

> [!NOTE]
>
> P4 控制接口独立于数据切换链路，始终连接，不受工作模式影响。
...
[协议文档](https://github.com/mixedsignal-labs/smartusbhub/blob/main/document/智能USB集线器_使用指南.md)
...
### 10.1 MODE_PC (ADB 调试) 验收标准

- P0 ←→ P1 导通，P2/P3 电气隔离
- PC 端可识别车机：`adb devices` 出现设备且为 `device` 状态
- `adb shell` 可执行基础命令（如 `getprop`）
- 切换到 MODE_PC 后，ADB 恢复时间 ≤ 10 秒

### 10.2 MODE_UDISK1 / MODE_UDISK2 (车机 Host 读 U 盘) 验收标准

- MODE_UDISK1: P0 ←→ P2 导通，P1/P3 电气隔离
- MODE_UDISK2: P0 ←→ P3 导通，P1/P2 电气隔离
- 车机识别 U 盘并可读取目录/文件
- 切换到 MODE_UDISK1/MODE_UDISK2 后，U 盘挂载时间 ≤ 10 秒（取决于Host协议栈）

### 10.3 互锁与供电安全验收标准

- 任意时刻仅一路导通（可通过示波器或万用表验证）
- 不允许 PC 与车机同时对同一路 VBUS 供电造成冲突
- 长时间常插（PC+U 盘同时插）下反复切换 100 次以上，系统不应出现不可恢复异常
